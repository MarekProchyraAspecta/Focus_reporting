...
def fn_SendMail_SendGridApp():
  ...
  print ( f"g_csvFilePath: {g_csvFilePath}" )
  print ( f"g_csvFileName: {g_csvFileName}" )
...
def fn_CommitCSVToGitHub():
  ...
  def fn_RestartSSHAgent():
    try:
        _l_private_key_path = '/home/opc/.ssh/aspecta_git'

        # Start the SSH agent
        agent_start = subprocess.Popen(['ssh-agent', '-s'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, error = agent_start.communicate()
        
        if agent_start.returncode != 0:
            print(f"Failed to start SSH agent: {error.decode().strip()}")
            return

        # Extract the environment variables for the SSH agent
        for line in output.decode().splitlines():
            if 'SSH_AUTH_SOCK' in line or 'SSH_AGENT_PID' in line:
                var_name, var_value = line.split(';')[0].split('=')
                os.environ[var_name] = var_value  # Set the environment variable

        # Add the private key to the SSH agent
        add_key = subprocess.Popen(['ssh-add', _l_private_key_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        add_key_output, add_key_error = add_key.communicate()
        
        if add_key.returncode == 0:
            print(f"SSH agent started and key added successfully from {_l_private_key_path}")
        else:
            print(f"Failed to add key: {add_key.stderr}")
...
def fn_Main():
  ...
  def fn_ProcessInputParams():
    ...
    parser.add_argument ( '-csv', action='store_true', default=True, dest='csv', help='Write to CSV files instead of output to the screen - usage_*.csv' )
